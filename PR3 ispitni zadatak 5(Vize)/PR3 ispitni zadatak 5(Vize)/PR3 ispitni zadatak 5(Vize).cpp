...
public:
	...
	Datum& operator++() {
		...
		else if (*_mjesec == 2 && *_dan == 28)
		{
			*_dan = 1;
			*_mjesec = 3;
		}
		else if (*_mjesec == 1 || *_mjesec == 3 || *_mjesec == 5 || *_mjesec == 7 || *_mjesec == 8 || *_mjesec == 10 || *_mjesec == 12 && (*_dan == 31))
		{
			*_dan = 1;
			if (*_mjesec == 12)
				*_mjesec = 1;
	...
	int GetYear()const { return *_godina; }
...
public:
	...
	FITKolekcija& operator=(const FITKolekcija& kol) {
		if (this != &kol) {
	...
	void Sortiraj(Sortiranje sort) {
		...
		while (promjena) {
			...
			{
				if ((sort == RASTUCI) ? *_elementi1[i] > *_elementi1[i + 1]:*_elementi1[i] < *_elementi1[i + 1]) {
...
public:
	...
	virtual ~Viza() { delete[] _napomena; _napomena = nullptr; }//dodajemo virtual ispred destruktora,jer po pravilu ako imamo bar 1 virtuelnu funkciju i destruktor mora biti virtual
...
public:
	...
	friend ostream & operator << (ostream & COUT, Aplikant & obj) {
		COUT << obj._imePrezime << " " << *obj._datumRodjenja << " " << obj._brojPasosa << endl;//potrebno dereferencirati datum
		COUT << "VIZE -> " << endl;
	...
	(POJAM VAŽEĆA PODRAZUMIJEVA DA JE POSLJEDNJI STATUS VIZE BIO IZDATA TJ. DA VIZA NIJE PONIŠTENA ILI ISTEKLA) VIZA ZA TU DRŽAVU, BEZ OBZIRA NA NJENU VRSTU.
	- JE NEKA OD RANIJIH VIZA PONIŠTENA (OVO SE ODNOSI SAMO NA VIZE ZA DRŽAVU: SAD)
	*/
	bool DodajVizu(Viza* v) {
		if (v->GetVazenjeOD().GetYear() - _datumRodjenja->GetYear() < 10) {
			return false;
		}
		for (size_t i = 0; i < _vize.size(); i++)
		{
			if (_vize[i]->GetDrzava() == v->GetDrzava()
				&& v->GetVazenjeOD().GetDays() >= _vize[i]->GetVazenjeOD().GetDays() &&
				v->GetVazenjeDO().GetDays() <= _vize[i]->GetVazenjeDO().GetDays()) {
				...
				for (size_t j = 0; j < _vize[i]->GetStatuse().GetTrenutno(); j++)
				{
					...
					if (_vize[i]->GetDrzava() == Drzava::SAD && *_vize[i]->GetStatuse().GetT1()[j] == Status::PONISTENA) {
						return false;
					}
				}
			}
	...
	*/
	...
	bool DodajStatus(Viza *v, Status s, string napomena = "") {
		int brojaciStatusa[6] = { 0 };
		for (size_t i = 0; i < _vize.size(); i++)
		{
			for (size_t j = 0; j < _vize[i]->GetStatuse().GetTrenutno(); j++)
			{
				brojaciStatusa[*_vize[i]->GetStatuse().GetT1()[j]]++;
			}
		}
		for (size_t i = 0; i < (int)s; i++)
		{
			if (brojaciStatusa[i] == 0)
				return false;
		}
	...
	void ZamijeniRijec(string trazeni, string novi) {
		regex r("(\\b" + trazeni + "\\b)|(\\B" + trazeni + "\\B)");
		for (size_t i = 0; i < _vize.size(); i++)
		{
			if (_vize[i]->GetNapomena() != nullptr) {
				string zamijeni(_vize[i]->GetNapomena());
				string zamijenjenaRijec = regex_replace(zamijeni, r, novi);
